...
// Player A to choose an asset that is avaialble in Pyth and inputs the length of the bet (CreateBet)
// Player B can see all existing bets (Fetch Bets)
// Player B can Match any bet and input his own prediction
// If the timing is over whoever is closest to the price can claim the WHOLE amount
//
...
declare_id!("3RHb7FsCPqfMD9X1e7skPTUYHZ8aYMqeEqGw9VuBDU7K");

#[program]
mod stock_prediction_dapp {
    use super::*;
    ...
    pub fn claim_bet(ctx: Context<ClaimBet>) -> Result<()> {
        // Check winner and send the price to winner OR return them back if it's a draw
        let bet = &mut ctx.accounts.bet;
